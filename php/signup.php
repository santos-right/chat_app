            ...
            if(mysqli_num_rows($sql) > 0){ // if email already exist 
                ...
                if(isset($_FILES['image'])){
                   ...
                   // $img_type = $_FILES['image']['type']; //getting user upload img type
                    ...
                    //let's explode image and get the last extensions like jpg png
                    $img_explode = explode('.', $img_name);
                    $img_ext = end($img_explode); //here we ge the extensions of a user upload img file

                    $extensions = ['png', 'jpeg', 'jpg']; //these are some valid img ext and we we've store them in an array
                    if(in_array($img_ext, $extensions) === true){ //if user upload img ext is matched with any array extensions
                        ...
                        if(move_uploaded_file($tmp_name, "../image/".$new_img_name)){ //if user upload move to our folder successfully
                            $status = "Active now"; //once user signed up then his satus will be active now
